    ...
    public partial class dlgManualInventory : Form
    {
        ...
        private void populateProduct()
        {
            using (MySqlConnection Con = new MySqlConnection(conf.connectionstring))
            {
                ...
                {
                    Con.Open();
                    string sqlQuery = "SELECT productId, productName,productSafetyStock FROM products WHERE status = @status ORDER BY productName ASC";
                    ...
                    while (reader.Read())
                    {
                        string[] dataRow = new string[] { reader["productId"].ToString(), reader["productName"].ToString(), reader["productSafetyStock"].ToString() };
