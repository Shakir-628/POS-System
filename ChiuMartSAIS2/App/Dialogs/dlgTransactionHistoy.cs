...
namespace ChiuMartSAIS2.App.Dialogs
{
    public partial class dlgTransactionHistoy : Form
    {
        ...
        {
            //action = "Print";
            Reports.frmOrReport rptT = new Reports.frmOrReport();
            rptT.orno = lstClients.SelectedItems[lstClients.SelectedItems.Count - 1].Text;
            rptT.ShowDialog();
        }
        ...
        private void btnPayBalance_Click(object sender, EventArgs e)
        {
            if (lstClients.SelectedItems.Count <= 0)
            {
                MessageBox.Show(this, "Please select an item.", "Notification", MessageBoxButtons.OK);
                return;
            }
